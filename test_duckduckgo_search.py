...
def test_empty_search_results(
    maximized_browser, duck_duck_go_page, not_too_long_random_chars
):
    browser.element('//input[@aria-label="Search with DuckDuckGo"]').should(
        be.blank
    ).type(not_too_long_random_chars).press_enter()
    browser.element("html").should(have.text("No results found"))


def test_too_long_search(maximized_browser, duck_duck_go_page, too_long_random_chars):
    browser.element('//input[@aria-label="Search with DuckDuckGo"]').should(
        be.blank
    ).type(too_long_random_chars).press_enter()
